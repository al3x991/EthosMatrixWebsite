...
const Footer = () => {
  ...
  const handleSubmit = async () => {
    ...
    // Subscribe the user using the react-mailchimp-subscribe library
    const response = await subscribe({ EMAIL: email });

    // Check the response status and set appropriate state
    ...
    render={({ subscribe, status, message }) => (
      <div>
        <div className="mt-0 md:mt-2 flex">
          ...
          />
          <button
            className="bg-amber-500 text-amber-950 px-4 py-2 hover:bg-gray-300 transition duration-300"
            onClick={() => subscribe({ EMAIL: email })} // Replace with user's email
          >
            Subscribe
          </button>
        </div>
